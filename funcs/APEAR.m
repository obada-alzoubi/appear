...
function [status] = APEAR( EEG, outdir, suffix)

% Inputs 
%- EEG          : EEG object from EEGLAB with APEAR structure 
...
[pkloc,~, ~, ~, ~,~] = cardperiod(EEG2, EEG2.APEAR.slice_per_TR/EEG2.APEAR.TR);
...
hold on; plot( Rpeaks,...
    ECG(Rpeaks),  'O');
xlim([polt_ecg_range(1)*EEG2.APEAR.Fs,polt_ecg_range(end)*EEG2.APEAR.Fs]);
...
switch EEG.APEAR.BCG_Crorrection 
    
    case 'Pulse_Ox'
        pusleox_waveform = EEG.APEAR.PulseOX.waveform;
        ...
        HR         = (EEG.APEAR.Fs./diff(EEG.APEAR.PulseOX.Peaks))*60;% in min
        meanHR3     = mean(HR);
  
        EEG                    = fmrib_pas(EEG2,EEG.APEAR.PulseOX.Peaks,'mean',21); %OZ: From Ahmad 
...
writematrix(strcat(outdir, '/', suffix, '_', 'HR_info.png'), EEG.APEAR.BCG_Crorrection);

% eeg-ecg separation
[EEG.data,EEG.nbchan,~,~] = eegecgsep(EEG.data,EEG.nbchan,EEG.pnts,EEG.APEAR.chlb);
...
% Save final EEG
corrEEG_filename = strcat(outdir, '/', suffix, '_', 'eeg_p-2');
% As edf
